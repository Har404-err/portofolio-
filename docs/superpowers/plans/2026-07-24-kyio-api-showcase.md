# Implementation Plan: KyioAPI Flagship Project Showcase

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Menambahkan seksi proyek unggulan `components/Projects.tsx` untuk mempromosikan KyioAPI (`https://api.kyio.web.id`) dengan card SaaS eksklusif, statistik 250+ endpoint, dan widget preview kode interaktif.

**Architecture:** Membuat komponen `components/Projects.tsx` yang selaras dengan tema portofolio, lalu mengintegrasikannya ke `App.tsx` dan `Navbar.tsx`.

**Tech Stack:** React 19, TypeScript, Framer Motion, TailwindCSS.

## Global Constraints
- Tidak ada AI slop (desain otentik, clean, typography tinggi).
- Mendukung switching snippet kode (`cURL`, `JavaScript`, `Python`).
- Menautkan link ke `https://api.kyio.web.id`.

---

### Task 1: Buat Komponen `components/Projects.tsx`
...
**Interfaces:**
- Consumes: None
- Produces: Komponen React `Projects`
...
- [ ] **Step 2: Commit new Projects component**

```bash
git add components/Projects.tsx
git commit -m "feat: add Projects component showcasing KyioAPI"
```

---

### Task 2: Integrasikan `Projects.tsx` ke `App.tsx` & Update `Navbar.tsx`
...
Import `<Projects />` dan posisikan di antara `<About />` dan `<Stack />`:

```tsx
import Projects from './components/Projects';

// inside App render main:
...
- [ ] **Step 2: Update `Navbar.tsx` link array**

Ubah array menu navigasi pada `Navbar.tsx` menjadi `['About', 'Projects', 'Stack', 'Contact']`.

- [ ] **Step 3: Verifikasi Build TypeScript**

Run: `npx -y typescript --noEmit` atau build check.

- [ ] **Step 4: Commit & Push**

```bash
git add App.tsx components/Navbar.tsx
git commit -m "feat: integrate Projects section into App and Navbar"
git push
```
