...
const MusicPlayer: React.FC = () => {
  ...
  return (
    <div className="fixed bottom-6 left-6 z-50 flex flex-col items-start gap-4">
      <AnimatePresence>
        {showSearch && (
            <motion.div 
                ...
                className="bg-[#0f172a]/90 backdrop-blur-xl border border-white/10 p-4 rounded-2xl w-72 shadow-2xl"
            >
                <form onSubmit={handleSearch} className="flex gap-2">
                    <input 
                        type="text" 
                        placeholder="Cari lagu..." 
                        className="bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-xs text-white w-full focus:border-[#00f0ff] focus:outline-none"
                        value={query}
                        onChange={(e) => setQuery(e.target.value)}
                    />
                    <button type="submit" disabled={loading} className="bg-[#00f0ff] text-black rounded-lg px-3 py-2 text-xs font-bold">
                        {loading ? '...' : 'GO'}
                    </button>
                </form>
            </motion.div>
      ...
      <motion.div
        layout
        className={`bg-[#0f172a]/80 backdrop-blur-xl border border-white/10 rounded-full flex items-center overflow-hidden transition-all duration-500 pr-2`}
        style={{ boxShadow: isPlaying ? '0 0 20px rgba(0, 240, 255, 0.2)' : 'none' }}
      >
        <button
          onClick={togglePlay}
        ...
        >
          ...
          />
          <div className="relative z-10 text-[#00f0ff] drop-shadow-md">
            {audioLoading ? (
               <svg className="animate-spin h-5 w-5 text-[#00f0ff]" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>
            ) : isPlaying ? (
        ...
        <div className="flex flex-col justify-center whitespace-nowrap ml-3 mr-2 overflow-hidden w-24 md:w-32">
            <span className="text-[10px] font-black uppercase text-[#00f0ff] tracking-wider truncate block w-full">{track.title}</span>
            <span className="text-[9px] text-white/50 font-mono truncate block w-full">{track.artist}</span>
        </div>

        <button 
            onClick={() => setShowSearch(!showSearch)}
            className={`p-2 rounded-full hover:bg-white/10 transition-colors ${showSearch ? 'text-[#00f0ff]' : 'text-white/50'}`}
        >
